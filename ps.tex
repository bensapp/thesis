...
foreground clutter.  We address these issues in \secref{stretchable}.

\subsection{Restricted pairwise potentials}

In order to achieve fast inference, modeling sacrifices have been made.

\mypar{Unimodality} The linear time max inference for PS described in 
\secref{dt} via distance transforms is supported only if the pairwise 
...
and lower arms, as shown in \figref{multi-modal-angles}. 

\mypar{Image independence} Not only must the pairwise potential be unimodal, it 
is also {\em not a function of the image}.  We  \todo{MAKE THIS A BIG POINT}
We address these issues beginning in \secref{CPS} for pairwise potentials, and 
\secref{llps} for pairwise {\em and} unary potentials.
