...
in~\figref{binary-multiclass-ml}, left.  When $k > 2$, we can also transform 
...
rest.  \figref{binary-multiclass-ml}, right illustrates an example with $k = 3$ 
and $d = 2$.
...
determine the spin of particles connected in a grid, and~\equref{mrf} describes 
the log of the energy of the system.  When given a probabilistic interpretation 
as we will see in~\secref{probinterp}, this type of model is known as a {\em 
...
and~\equref{mrf} as the negative of an {\em energy function} which we seek to 
...
locations over 100 time steps, see~\figreff{mrf-examples}{a}.  We have no idea 
...
chain, seen in~\figreff{mrf-examples}{b}.
...
image, giving us a cyclic planar graph~\figreff{mrf-examples}{c}.

\mypar{Human pose estimation}  Knowing the location of the left shoulder is 
...
see~\figreff{mrf-examples}{d} and~\secref{ps}.
...
separating hyperplane an example lies on (\secref{binary-multi-class})---and 
...
impose on our models, discussed in~\secref{mrfs}, allows us to find a solution 
...
example of the robot localization problem~\equref{nesting}, we can perform 
...
completeness, the algorithm is in \algref{max-inference}, commonly referred to 
...
\secref{probinterp}), we can consider the marginal distribution of a particular 
variable
\begin{equation}
...
$j$ attached to $i$, then run~\algref{max-inference}.  This ensures that $y_i$ 
...
\end{equation}
which is the maximizer which corresponds to maximum $\mmi$.  For completeness, the algorithm to compute max-marginal values and witness statistics is included in~\secref{} in~\algref{mm-inference}.

\section{Supervised learning}\secref{sec:learning}
