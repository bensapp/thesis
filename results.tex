...
color and contours that were intractable to compute without our cascade 
approach.  In the bar marked ``baseline PS'' we evaluate a classical unimodal 
PS, whereas the geometric parameters in our cascade models (``geom only'') are 
learned bin weights that can achieve non-linearity.  All features together do 
significantly better than any one feature modality on top of the standard 
